...
export function BusinessIntelligence() {
  ...
  const fetchCostBreakdown = async () => {
    try {
      const response = await fetch(`/api/admin/cost-breakdown?range=${timeRange}`);
      if (response.ok) {
        const data = await response.json();
  ...
  if (!metrics) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">Unable to load metrics</h3>
      </div>
    );
  }

  const profitMargin = metrics.revenue.total > 0 
    ? ((metrics.revenue.total - metrics.ai_performance.total_cost) / metrics.revenue.total) * 100
    : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Key performance indicators and financial metrics</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center mt-1">
                ...
                <span className={`text-sm ${metrics.revenue.growth >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  {formatPercentage(Math.abs(metrics.revenue.growth))} vs last period
                </span>
              </div>
            </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Avg score: {metrics.ai_performance.avg_score?.toFixed(1) || 'N/A'}
              </p>
            </div>
        ...
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <TrendingUpIcon className={`h-8 w-8 ${profitMargin > 50 ? 'text-green-500' : profitMargin > 20 ? 'text-yellow-500' : 'text-red-500'}`} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Cost Breakdown */}
        <div className="bg-white p-6 rounded-lg border border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">AI Cost Breakdown</h3>
            ...
            <div className="p-3 bg-blue-50 rounded-lg">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-blue-900">
                  Revenue per user: {formatCurrency(metrics.revenue.total / Math.max(metrics.users.total, 1))}
                </span>
              </div>
            </div>
            ...
            <div className="p-3 bg-purple-50 rounded-lg">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-purple-900">
                  Avg response time: {metrics.ai_performance.avg_response_time}ms
                </span>
              </div>
            </div>
            ...
            <div className="p-3 bg-green-50 rounded-lg">
              <div className="flex items-center">
                ...
                <span className="text-sm font-medium text-green-900">
                  Cost per request: {formatCurrency(metrics.ai_performance.cost_per_request)}
                </span>
              </div>
            </div>
              ...
              <div className="p-3 bg-yellow-50 rounded-lg">
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-yellow-900">
                    Conversion rate: {formatPercentage(metrics.users.conversion_rate)}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="space-y-3">
            {profitMargin < 20 && (
              <div className="flex items-start p-3 bg-red-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-red-900">Low Profit Margin</p>
                  <p className="text-xs text-red-700">Current margin is {profitMargin.toFixed(1)}%</p>
                </div>
              </div>
            )}
            ...
            {metrics.ai_performance.success_rate < 85 && (
              <div className="flex items-start p-3 bg-yellow-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-yellow-900">AI Performance</p>
                  <p className="text-xs text-yellow-700">Success rate is {metrics.ai_performance.success_rate.toFixed(1)}%</p>
                </div>
              </div>
            )}
            ...
            {metrics.ai_performance.avg_response_time > 3000 && (
              <div className="flex items-start p-3 bg-orange-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-orange-900">Slow Response Time</p>
                  <p className="text-xs text-orange-700">Avg: {metrics.ai_performance.avg_response_time}ms</p>
                </div>
              </div>
            )}
            ...
            {metrics.users.churn_rate && metrics.users.churn_rate > 0.1 && (
              <div className="flex items-start p-3 bg-red-50 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-red-900">High Churn Rate</p>
                  <p className="text-xs text-red-700">Churn: {formatPercentage(metrics.users.churn_rate)}</p>
                </div>
              </div>
            )}
