...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: feedback, error: feedbackError } = await supabase
      .from('feedback')
      .select('score, cost_usd, response_time_ms, user_rating, error_message, created_at')
      .gte('created_at', thirtyDaysAgo.toISOString());
    ...
    const responseTimes = feedback?.filter(f => f.response_time_ms).map(f => f.response_time_ms) || [];
    const avgResponseTime = responseTimes.length > 0
      ? responseTimes.reduce((sum, time) => sum + time, 0) / responseTimes.length
      : 0;
    
    const errorCount = feedback?.filter(f => f.error_message).length || 0;
