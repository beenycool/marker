...
export async function GET(request: NextRequest) {
  try {
    ...
    const activeUsers = users?.filter(u => 
      u.last_sign_in_at && new Date(u.last_sign_in_at) > new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000)
    ).length || 0;
    ...
    const validFeedback = feedback?.filter(f => f.score !== null) || [];
    const avgScore = validFeedback.length > 0 
    ...
    const avgResponseTime = feedback?.length
      ? feedback.reduce((sum, f) => sum + (f.response_time_ms || 0), 0) / feedback.length
      : 0;

    const totalCost = feedback?.reduce((sum, f) => sum + (f.cost_usd || 0), 0) || 0;
    const costPerRequest = totalSubmissions > 0 ? totalCost / totalSubmissions : 0;
